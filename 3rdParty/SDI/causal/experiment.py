        ...
        for q in self.brk(range(self.a.dpe)):
            ...
            if self.a.train_functional:
                ...
                for b, (batch, config) in self.brk(enumerate(zip(smpiter, cfgiter)), max=self.a.train_functional):
                    ...
                    self.S.model.W0gt.grad.mul_(-1.0/self.S.model.M)
                    self.S.model.B0gt.grad.mul_(-1.0/self.S.model.M)
                    self.S.model.W1gt.grad.mul_(-1.0/self.S.model.M)
                    self.S.model.B1gt.grad.mul_(-1.0/self.S.model.M)
